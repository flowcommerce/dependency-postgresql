...
-- Start of file https://github.com/flowcommerce/lib-postgresql/blob/master/schema/pg_partman--2.2.2.sql
...
CREATE INDEX part_config_type_idx ON partman.part_config (partition_type);
...
IF p_child_table IS NULL THEN
    ...
    LOOP
        IF (v_type = 'time' OR v_type = 'time-custom') THEN
            ...
            IF v_max_timestamp IS NOT NULL THEN
                SELECT suffix_timestamp FROM partman.show_partition_name(p_parent_table, v_max_timestamp::text) INTO v_last_partition_timestamp;
        ...
        ELSIF v_type = 'id' THEN
            ...
            IF v_max_id IS NOT NULL THEN
                SELECT suffix_id FROM partman.show_partition_name(p_parent_table, v_max_id::text) INTO v_last_partition_id;
    ...
    v_child_table := v_parent_schema ||'.'|| partman.check_name_length(v_parent_tablename, v_partition_suffix, TRUE);
...
*/
CREATE FUNCTION partman.check_subpart_sameconfig(p_parent_table text)
    ...
    IF v_run_maint IS FALSE THEN
        ...
        IF (NEW.%I %% %s) > (%s / 2) THEN
            ...
            WHILE ((v_next_partition_id - v_current_partition_id) / %s) <= %s LOOP
                v_partition_created := partman.create_partition_id(%L, ARRAY[v_next_partition_id]);
                IF v_partition_created THEN
                    PERFORM partman.create_function_id(%L);
...
FOR v_row IN
    SELECT sub_parent FROM partman.part_config_sub
LOOP
    SELECT count(*) INTO v_check_subpart FROM partman.check_subpart_sameconfig(v_row.sub_parent);
...
LOOP
    ...
    IF v_row.partition_type = 'time' OR v_row.partition_type = 'time-custom' THEN
        ...
        IF v_max_time_parent > v_current_partition_timestamp THEN
            SELECT suffix_timestamp INTO v_current_partition_timestamp FROM partman.show_partition_name(v_row.parent_table, v_max_time_parent::text);
        END IF;
    ...
    PERFORM partman.apply_constraints(p_parent_table := v_row.parent_table, p_job_id := v_job_id, p_debug := p_debug);
...
END LOOP;
FOR v_row IN
    SELECT parent_table FROM partman.part_config WHERE retention IS NOT NULL AND undo_in_progress = false AND partition_type = 'id'
LOOP
    IF p_parent_table IS NULL THEN
...
WHILE v_batch_loop_count < p_batch_count LOOP
    -- Get ordered list of child table in set. Store in variable one at a time per loop until none are left.
    SELECT partition_tablename INTO v_child_table FROM partman.show_partitions(p_parent_table, 'ASC');
